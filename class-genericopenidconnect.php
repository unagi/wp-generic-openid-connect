 ...
 *
 * Connect to OpenID Connect IdP with Authorization Code Flow
 * Text Domain: generic-openid-connect
 * Domain Path: /language
 *
 * @version   1.0.1
 * @package   Wordpress\GenericOpenIDConnect
 * @license   GPLv2
 * @copyright (c) 2014-2017 shirounagi
 * @author    shirounagi
 * @link      https://github.com/unagi/wp-generic-openid-connect
 */
...
class GenericOpenIDConnect {
	...
	/**
	 * Wrap wp_request for this plugin
	 *
	 ...
	 * @param String  $authed_username  Authorized username.
	 */
	...
	/**
	 * Check option - used by launchkey_page_init
	 *
	 * @param  array $input input items.
	 * @return array
	 */
	 ...
	 * Print text field
	 *
	 * @param array $args key, css_class, add_opt.
	 */
	...
	public function print_bool_field( $key ) {
		echo '<input type="checkbox" id="' . $key . '" name="array_key[' . $key . ']" value="1" ' . ('1' == $this->$key ? 'checked="checked"' : '' ) . '>';
	}
	...
	private function styled_error_message( $errno ) {
		$message = self::$error_messages[ $errno ];
	...
	/**
	 * OpenID Connect app settings section info
	 */
